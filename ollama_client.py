...
def enforce_risk_rules(data: dict) -> dict:
    ...
    if ml_should_run:
        ml_log_lines = []
        # Collect signal context for informative header
        _ml_sym = str(data.get("symbol") or data.get("symbol_id") or "?").strip()
        _ml_price = data.get("price")
        _sd = data.get("state_diff") if isinstance(data.get("state_diff"), dict) else {}
        _ml_tf = str(
            data.get("active_reference_tf") or data.get("reference_tf")
            or _sd.get("active_reference_tf") or "?"
        ).strip()
        _ml_price_str = f"{_ml_price:.4f}" if isinstance(_ml_price, (int, float)) else str(_ml_price or "?")
        ml_log_lines.append(
            f"ML FILTER [PHASE1] {_ml_sym} | {signal_status_ml} | {_ml_tf} | price={_ml_price_str}"
        )
        ...
        try:
            ...
            if os.path.isfile(model_path):
                                    ...
                                    if not bars_df.empty:
                                        ...
                                        if len(closes) >= 2:
                                            ...
                                            tf_ord = tf_map.get(ref_tf.lower(), 3)
                                                                                        ...
                                                                                        # --- Enhanced features from OHLCV (+13) ---
                                            _hh = sum(1 for i in range(1, min(len(closes), 11)) if closes[-i] > closes[-i-1]) if len(closes) >= 2 else 0
                                            _ll = sum(1 for i in range(1, min(len(closes), 11)) if lows[-i] < lows[-i-1]) if len(lows) >= 2 else 0
                                            _ub = sum(1 for i in range(1, min(len(closes), 11)) if closes[-i] > opens[-i])
                                            higher_highs_ratio = float(_hh / min(len(closes)-1, 10))
                                            lower_lows_ratio = float(_ll / min(len(lows)-1, 10))
                                            up_bars_ratio = float(_ub / min(len(closes)-1, 10))

                                            return_3bar = float(((closes[-1] / closes[-4]) - 1) * 100) if len(closes) >= 4 else 0.0
                                            return_6bar = float(((closes[-1] / closes[-7]) - 1) * 100) if len(closes) >= 7 else 0.0

                                            if len(closes) >= 2:
                                                prev_body = abs(closes[-2] - opens[-2])
                                                curr_body = abs(closes[-1] - opens[-1])
                                                engulfing_strength = float(curr_body / (prev_body + 1e-9))
                                            else:
                                                engulfing_strength = 0.0

                                            _n_high = float(np.max(highs[-min(20, len(highs)):]))
                                            _n_low = float(np.min(lows[-min(20, len(lows)):]))
                                            dist_to_high_pct = float((_n_high - closes[-1]) / (closes[-1] + 1e-9) * 100)
                                            dist_to_low_pct = float((closes[-1] - _n_low) / (closes[-1] + 1e-9) * 100)

                                            if len(closes) >= 12:
                                                _range_prev = np.mean(high_low_ranges[-12:-6]) if len(high_low_ranges) >= 12 else atr_proxy
                                                _range_curr = np.mean(high_low_ranges[-6:]) if len(high_low_ranges) >= 6 else atr_proxy
                                                range_contraction = float(_range_curr / (_range_prev + 1e-9))
                                            else:
                                                range_contraction = 1.0

                                            if len(closes) >= 11:
                                                _roc_fast = (closes[-1] / closes[-6] - 1) * 100
                                                _roc_slow = (closes[-6] / closes[-11] - 1) * 100
                                                price_accel = float(_roc_fast - _roc_slow)
                                            else:
                                                price_accel = 0.0

                                            if len(closes) >= 20:
                                                _sma5_vs_20 = (sma_5 / (sma_20 + 1e-9) - 1) * 100
                                                _vol_norm = float(np.std(pct_changes[-14:])) * 100 if len(pct_changes) >= 14 else 0
                                                regime_score = float(np.clip(_sma5_vs_20 * 10 - _vol_norm, -100, 100))
                                            else:
                                                regime_score = 0.0

                                            if len(closes) >= 10:
                                                _cp = (closes - np.mean(closes))
                                                _cv = (volumes - np.mean(volumes))
                                                _cn = np.sum(_cp * _cv)
                                                _cd = np.sqrt(np.sum(_cp**2) * np.sum(_cv**2) + 1e-12)
                                                pv_correlation = float(_cn / _cd)
                                            else:
                                                pv_correlation = 0.0

                                            _rsi_curr = float(50 + np.clip(np.mean(pct_changes[-14:]) * 1000, -50, 50)) if len(pct_changes) >= 3 else 50.0
                                            _rsi_prev = float(50 + np.clip(np.mean(pct_changes[-15:-1]) * 1000, -50, 50)) if len(pct_changes) >= 15 else 50.0
                                            rsi_momentum = float(_rsi_curr - _rsi_prev)

                                            context_dict = {
                                                # Core OHLCV (28) — fixed vol_std_pct and volatility_pct
                                                "atr": float(atr_proxy),
                                                "atr_pct": float(atr_proxy / (closes[-1] + 1e-9) * 100),
                                                "bb_width": float(bb_width * 100),
                                                "body_ratio": float(body_ratio),
                                                "roc_5": float(roc_5),
                                                "row_atr_pct": float(candle_range / (atr_proxy + 1e-9) * 100),
                                                "vol_avg": float(vol_mean),
                                                "vol_std": vol_std_val,
                                                "vol_std_pct": float(vol_std_val / (vol_mean + 1e-9) * 100),
                                                "vol_trend": float(vol_trend),
                                                "vol_ratio_row": float(vol_ratio),
                                                "volatility_pct": float(np.std(pct_changes) * 100) if len(pct_changes) > 0 else 0.0,
                                                ...
                                                # Enhanced (+13 — recovers ~21% model weight)
                                                "higher_highs_ratio": higher_highs_ratio,
                                                "lower_lows_ratio": lower_lows_ratio,
                                                "up_bars_ratio": up_bars_ratio,
                                                "return_3bar": return_3bar,
                                                "return_6bar": return_6bar,
                ...
                if expected_features and len(flat) > 0:
                    ...
                    _ml_verdict = "PASS" if confidence >= 0.75 else "FILTERED"
                    data["ml_prediction"] = int(1 if confidence >= 0.75 else 0)
                    data["ml_model_type"] = type(clf_step).__name__
                    _ml_matched = sum(1 for f in expected_features if f in flat)
                    data["ml_features_used"] = len(expected_features)
                    data["ml_features_matched"] = _ml_matched
                    data["ml_phase"] = "PHASE1_LOG_ONLY"

                    # Summary line
                    ml_log_lines.append(
                        f"  → {_ml_verdict} | confidence={confidence:.1%} | "
                        f"model={type(clf_step).__name__} | features={_ml_matched}/{len(expected_features)}"
                    )
...
def format_json_for_tg(data: dict) -> str:
    ...
    if ml_confidence is not None:
        ...
        ml_sym_tg = str(data.get("symbol") or data.get("symbol_id") or "?").strip()
        ml_sig_tg = str(data.get("signal_status", "?")).replace("_", " ").capitalize()
        ml_tf_tg = "?"
        _sd_tg = data.get("state_diff") if isinstance(data.get("state_diff"), dict) else {}
        ml_tf_tg = str(
            data.get("reference_tf") or data.get("active_reference_tf")
            or _sd_tg.get("active_reference_tf") or "?"
        ).strip()
        ml_price_tg = data.get("price") or data.get("current_price") or "?"
        if isinstance(ml_price_tg, (int, float)):
            # Compact: no decimals for >100, 2 decimals for <100
            if ml_price_tg >= 100:
                ml_price_tg = f"{ml_price_tg:,.0f}"
            else:
                ml_price_tg = f"{ml_price_tg:.2f}"
        lines.append(f"🤖 ML [{data.get('ml_phase', 'PHASE1').replace('_LOG_ONLY', '')}] {ml_sym_tg} | {ml_sig_tg} | {ml_tf_tg} | {ml_price_tg}")
        lines.append(f"   → {verdict} | confidence={ml_confidence:.1%} | {ml_model} | features={ml_matched}/{ml_used}")
